...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords)
{
    ...
    // quantize each rgb channel to nearest web-safe color (0.0, 0.2, ..., 1.0)
    tex.rgb = round_vec3(tex.rgb * 5.0) / 5.0;
    // force usage with basically no impact
    tex.rgb += vec3(0.00001) * websafe.y;
